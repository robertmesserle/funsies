...
export class Board {
  ...
  get valid() {
    return this.validateBy('row') && this.validateBy('col') &&
           this.validateBy('sec');
  }
    ...
    function handleSolveAttempt(parent: Board, board: Board) {
      if (board.complete) {
        ...
        if (attempt < candidates.length - 1)
          parent.solve(attempt + 1, cellNumber);
        else if (cellNumber < set.length - 1)
          parent.solve(0, cellNumber + 1);
        else if (depth === 0) {
  ...
  parseBoard() {
    ...
    board.data.cells.forEach((cell, index) =>
                                 this.data.cells[index].cloneFrom(cell));
  }

  reset() { this.data = Board.createCells(); }

  fillWithEasy() {
    this.fillBoard(
        '-----8--4-84-16------5--1--1-38--9--6-8---4-3--2--95-1--7--2------78-26-2--3-----');
  }
  ...
  fillWithDifficult(number: number) {
    ...
    case 1:
      return this.fillBoard(
          '86--2-------7---59-------------6-8---4---------53----7----------2----6----75-9---');
    case 2:
      return this.fillBoard(
          '----8-2-4--------5-826-4------8---3--6-9-2-4--7---5------3-962-7--------1-3-4----');
    }
  }
  ...
  fillBoard(str: string) {
    ...
    this.data.cells.forEach((cell, index) => {
      ...
      if (!isNaN(Number(char)))
        cell.handleClick(str.charAt(index));
    });
  }
  ...
  clone() {
    let board = new Board();
    board.data.cells.forEach((cell, index) =>
                                 cell.cloneFrom(this.data.cells[index]));
    return board;
  }
  ...
  static createCells() {
    ...
    for (let row = 0; row < 9; row++) {
      for (let col = 0; col < 9; col++) {
        let section = parseInt(
            String(Math.floor(row / 3)) + String(Math.floor(col / 3)), 3);
    ...
    data.cells.forEach(
        (cell: Cell) => cell.siblings = data.cells.filter(
            c => c !== cell && (c.row == cell.row || c.col == cell.col ||
                                c.sec == cell.sec)));
    return data;
  }
}
